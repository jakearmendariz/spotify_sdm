...
def success():
    ...
    if response.status_code != 200:
        ...
        for item in response['items']:
            print(item['track']['id'],'is', item['track']['name'])
...
def read_daily():
    global spotify
    response = requests.get(f'https://api.spotify.com/v1/playlists/{NAMING}/tracks', 
        headers = spotify.access_token_headers())
    print('sending headers', spotify.access_token_headers())
    if response.status_code != 200:
        return json.dumps({'error': response.reason}), response.status_code
        ...
        response = response.json()
        return json.dumps({'success':'read daily mix', 'response': response}), 200
